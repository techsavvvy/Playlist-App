...
class PlaylistCreateForm(FlaskForm):
    name = StringField('Playlist Name', validators=[
                       InputRequired(), Length(max=99)])
    genre = StringField('Genre', validators=[
                        InputRequired(), Length(min=1, max=99)])
...
class CollectionsCreateForm(FlaskForm):
    title = StringField('Title', validators=[InputRequired(
        message="dsfgsfas"), Length(min=1, max=99)])
...
class CollectionSearchForm(FlaskForm):
    search = StringField(
        '',
        validators=[InputRequired(), Length(min=1, max=50)],
        render_kw={"placeholder": "search here using title / album / artist"}
    )
